  ...
  for (i in 1:3){
    ...
    X_build <- subset(train,(Applicant_Gender==i),select = -c(ID) )
     model_rf <- randomForest(train$Business_Sourced ~ train$Applicant_City_PIN, data=X_build,ntree=576)
    
    #X_Train<- subset(test,(test$Applicant_Gender==i),select = -c(ID)  )
    #pred_rf <- predict(model_rf, X_Train[,-1])
...
# m<- matrix()
#    m[1]=c("Male"=max(model_rf$err.rate))
#    m[2]=c("FeMale"=max(model_rf$err.rate))
#  dd<-data.frame(m)
